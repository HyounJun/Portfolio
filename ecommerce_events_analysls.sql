...
WITH view_person as ( --제품을 본 사람  
        ...
        from 2019_dec
        where event_type = 'cart'
),remove_person as ( -- 장바구니에서 삭제한사람
        SELECT event_time
        ,user_id
        ,user_session as remove_at
        from 2019_dec
        where event_type = 'remove_from_cart'
)
SELECT count(DISTINCT view_person.user_id , view_person.view_at) as view_cnt
       ,count(DISTINCT cart_person.user_id , cart_person.cart_at) as cart_after
       ,count(DISTINCT remove_person.user_id , remove_person.remove_at) as remove_at_after
from view_person
    left join cart_person on view_person.user_id = cart_person.user_id
                         AND view_person.view_at = cart_person.cart_at
                         AND view_person.event_time <= cart_person.event_time -- 제품을 본 세션과 장바구니에 담은세션이 같은것도 포함
    left JOIN remove_person on remove_person.user_id = cart_person.user_id
                             AND remove_person.remove_at = cart_person.cart_at
                             AND remove_person.event_time >= cart_person.event_time -- 장바구니에 담은 뒤 삭제
